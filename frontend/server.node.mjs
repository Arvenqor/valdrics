...
if (process.env.ORIGIN && !configuredOrigin) {
	throw new Error('frontend_runtime_invalid_origin_env');
}

if (process.env.NODE_ENV === 'production' && !configuredOrigin && !(protocolHeader && hostHeader)) {
	console.warn('frontend_runtime_origin_not_explicitly_configured');
}
...
const server = createServer(async (req, res) => {
	try {
		...
		console.error('frontend_runtime_request_failed', error);
		if (!res.headersSent) {
			res.statusCode = 500;
...
server.listen(port, host, () => {
	console.log(`frontend_runtime_listening http://${host}:${port}`);
});
