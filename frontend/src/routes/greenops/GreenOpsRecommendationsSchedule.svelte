...
{#if carbonData}
	<div class="material-card-3d p-6 mt-8 relative overflow-hidden group">
		<h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
			...
			Recommended Regions
			<span
				class="text-xs font-semibold text-emerald-300 bg-emerald-950/40 px-2.5 py-0.5 rounded border border-emerald-500/20"
		...
		{#if (carbonData.green_region_recommendations?.length ?? 0) > 0}
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				{#each (carbonData.green_region_recommendations ?? []).slice(0, 3) as rec (rec.region)}
					<div class="green-gradient-card p-4 rounded-xl transition-all duration-200">
						<div class="flex justify-between items-start">
							<span class="font-bold text-white transition-colors">{rec.region}</span>
							<span
								class="text-xs bg-emerald-950/50 text-emerald-300 font-bold px-2 py-0.5 rounded border border-emerald-500/20"
								>{rec.carbon_intensity} g/kWh</span
							>
						</div>
						<div class="mt-3 text-sm text-ink-400 font-medium">
							Reduce emissions by <span class="text-emerald-400 font-extrabold"
								>-{rec.savings_percent}%</span
							>
						</div>
					</div>
				{/each}
...
<div class="material-card-3d p-6 mt-8 relative overflow-hidden">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
		<div>
			...
			<p class="text-ink-400 text-sm mt-1">
				Schedule non-urgent batch jobs during lower carbon intensity windows.
			</p>
		</div>
		<div>
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
		...
		<div
			class="bg-ink-950/40 p-6 rounded-2xl border border-ink-800/80 flex flex-col justify-between"
		>
			<div>
				...
				<div class="space-y-5">
					<div>
						<div class="flex justify-between items-baseline mb-2">
							<label for="duration" class="text-xs font-medium text-ink-400">Job Duration</label>
							<span
								class="text-xs font-mono font-bold text-white bg-ink-950 border border-ink-800 px-1.5 py-0.5 rounded"
								>{workloadDuration} hours</span
							>
						</div>
						<input
							type="range"
			...
			{#if scheduleResult}
				...
				>
					<div class="text-[10px] uppercase font-extrabold text-emerald-400 tracking-wider mb-1">
						Optimal Recommendation
					</div>
					<div class="text-sm text-white font-semibold leading-relaxed">
						{scheduleResult.recommendation}
					</div>
				</div>
			{/if}
		</div>
