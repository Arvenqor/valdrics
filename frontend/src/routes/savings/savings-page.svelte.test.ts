...
const { getMock, loggerErrorMock } = vi.hoisted(() => ({
	getMock: vi.fn(),
...
describe('savings proof page', () => {
	...
	it('renders savings proof breakdown from API', async () => {
		...
		expect(await screen.findByText('idle_instances')).toBeTruthy();
		expect(await screen.findByText('i-123')).toBeTruthy();
	...
	it('keeps the proof summary visible when a child report fails', async () => {
		getMock.mockImplementation(async (url: string) => {
			if (url.includes('/savings/proof/drilldown')) {
				return jsonResponse({ detail: 'Drilldown backend unavailable.' }, 503);
			}
			if (url.includes('/savings/realized/events')) {
				return jsonResponse([]);
			}
			return jsonResponse({
				...
				as_of: '2026-02-13T00:00:00Z',
				tier: 'pro',
				opportunity_monthly_usd: 120,
				realized_monthly_usd: 75,
				open_recommendations: 2,
				applied_recommendations: 1,
				pending_remediations: 1,
		...
		expect(await screen.findByText('Realized monthly')).toBeTruthy();
		expect(screen.getAllByText('AWS').length).toBeGreaterThan(0);
		expect(await screen.findByText('Drilldown backend unavailable.')).toBeTruthy();
		expect(screen.getByText(/Available report data remains visible/i)).toBeTruthy();
	});

	it('renders empty provider, drilldown, and evidence states', async () => {
		getMock.mockImplementation(async (url: string) => {
			if (url.includes('/savings/proof/drilldown')) {
				return jsonResponse({
					start_date: '2026-02-01',
					...
					provider: null,
					dimension: 'strategy_type',
					opportunity_monthly_usd: 0,
					realized_monthly_usd: 0,
					buckets: [],
					truncated: false,
					limit: 50,
					notes: []
		...
		expect(await screen.findByText('No provider savings found for this window.')).toBeTruthy();
		expect(await screen.findByText('No recommendation buckets matched this filter.')).toBeTruthy();
		expect(
			await screen.findByText('No realized savings evidence events were found for this window.')
		).toBeTruthy();
	});
});
