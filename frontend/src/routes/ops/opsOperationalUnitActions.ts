	...
	async function saveUnitEconomicsSettings(event?: SubmitEvent) {
		...
		try {
			...
			const payload = {
				...
				target_spend_reduction_pct: Number(state.unitSettings.target_spend_reduction_pct),
				target_rollout_days: Number(state.unitSettings.target_rollout_days),
				target_team_members: Number(state.unitSettings.target_team_members),
				target_blended_hourly_rate: Number(state.unitSettings.target_blended_hourly_rate)
			};
