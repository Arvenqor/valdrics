...
{#if open && selectedRequest}
	<div class="fixed inset-0 z-[150] flex items-center justify-center p-4">
		<button
			...
			onclick={onClose}
		></button>
		<div class="material-perspective w-full max-w-2xl relative">
			<div
				class="card material-card-3d p-6"
				role="dialog"
				aria-modal="true"
			...
			>
				...
				<div class="space-y-3 text-sm">
					...
					{#if policyPreviewLoading}
						<div class="card border border-ink-700/60 bg-ink-900/30 p-3">
					...
					{:else if selectedPolicyPreview}
						...
						{#if selectedPolicyPreview.rule_hits.length > 0}
							<div class="rounded-lg border border-ink-700/60 p-3 bg-ink-900/20">
								...
								<ul class="space-y-1 text-xs text-ink-300">
									{#each selectedPolicyPreview.rule_hits as hit (hit.rule_id)}
										<li>
											<span class="font-semibold text-ink-200">{hit.rule_id}</span>
											{#if hit.message}
												: {hit.message}
											{/if}
										</li>
					...
					{#if remediationModalError}
						<div class="card border-danger-500/50 bg-danger-500/10 p-3">
							<p class="text-danger-400 text-xs">{remediationModalError}</p>
						</div>
					{/if}

					{#if remediationModalSuccess}
						<div class="card border-success-500/50 bg-success-500/10 p-3">
				...
				<div class="mt-5 flex items-center justify-end gap-2">
					{#if selectedRequest.status === 'approved' || selectedRequest.status === 'scheduled'}
						<label class="flex items-center gap-2 text-xs text-ink-400 mr-auto cursor-pointer">
							<input type="checkbox" class="toggle" bind:checked={bypassGracePeriod} />
							Bypass grace period
						</label>
					{/if}
					<button
