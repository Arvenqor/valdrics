<script lang="ts">
	...
	import type ConnectionsInventoryReconciliationComponent from './ConnectionsInventoryReconciliation.svelte';

	type ConnectionsInventoryReconciliationProps = ComponentProps<
		typeof ConnectionsInventoryReconciliationComponent
	>;
	...
	const loadInventoryReconciliation = createLazyComponent<ConnectionsInventoryReconciliationProps>(
		() => import('./ConnectionsInventoryReconciliation.svelte')
	);
...
<AuthGate authenticated={!!data.user} action="manage cloud accounts">
	<div class="connections-page">
		...
		{#await loadInventoryReconciliation()}
			<div class="card connections-page__inventory-placeholder">
				<div class="skeleton h-8 w-64 mb-4"></div>
				<div class="skeleton h-28 rounded-2xl"></div>
			</div>
		{:then module}
			{@const ConnectionsInventoryReconciliation = module.default}
			<ConnectionsInventoryReconciliation
				{loadingAWS}
				{loadingAzure}
				{loadingGCP}
				{loadingSaaS}
