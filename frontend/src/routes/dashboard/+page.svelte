<script lang="ts">
	...
	import PremiumDashboardOverview from '$lib/components/dashboard/PremiumDashboardOverview.svelte';
	...
	let unitEconomicsSettings = $derived(data.unitEconomicsSettings);
	let leadershipKpis = $derived(data.leadershipKpis);
	let dailyCosts = $derived(data.dailyCosts);
	let approvalsQueue = $derived(data.approvalsQueue);
	let freshness = $derived(data.freshness);
	let error = $derived(data.error || '');
	let warning = $derived(data.warning || '');
	...
	let overview = $derived(
		buildDashboardOverview({
			costs,
			carbon,
			zombies: zombies as ZombieCollections | null | undefined,
			leadershipKpis,
			dailyCosts,
			approvalsQueue,
			periodLabel,
	...
	<div class="dashboard-page">
		<div class="dashboard-command">
			<div class="dashboard-command__header">
				<div>
					<h1>{personaContent.title} Dashboard</h1>
					<p>{personaContent.subtitle}</p>
				</div>
		...
		{:else}
			...
			<PremiumDashboardOverview {overview} />

			<div class="dashboard-next-actions stagger-enter" style="animation-delay: 160ms;">
				<div class="dashboard-next-actions__body">
					<div>
						<h2>Next actions</h2>
						<p>{personaContent.nextActionsCopy}</p>
					</div>
			...
			{#if persona === 'finance' || persona === 'leadership'}
				{#if financeDashboardSection}
					...
					<FinanceDashboardSection {allocation} {tier} {unitEconomics} {unitEconomicsSettings} />
