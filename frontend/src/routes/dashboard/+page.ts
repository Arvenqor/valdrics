...
export const load: PageLoad = async ({ fetch, parent, url }) => {
	...
	try {
		...
		const widgetRequests: Array<{
			...
			{
				// Fetch KPI target settings in parallel — used by FinanceDashboardSection
				// to surface ROI planner context. Available to all tiers, no date params.
				key: 'unitEconomicsSettings',
				label: 'unit economics settings',
				request: wantsUnitEconomics
					? getWithTimeout(edgeApiPath('/costs/unit-economics/settings'))
					: Promise.resolve(null)
			},
			{
				key: 'leadershipKpis',
			...
			},
			{
				...
				request: ['pro', 'enterprise'].includes(tier)
					? getWithTimeout(edgeApiPath('/enforcement/approvals/queue?limit=50'))
					: Promise.resolve(null)
			}
		];
		...
		const unitEconomicsSettingsRes = widgetResponses.get('unitEconomicsSettings') ?? null;
		const leadershipKpisRes = widgetResponses.get('leadershipKpis') ?? null;
		const dailyCostsRes = widgetResponses.get('dailyCosts') ?? null;
		const approvalsQueueRes = widgetResponses.get('approvalsQueue') ?? null;
		...
		const unitEconomicsSettings =
			unitEconomicsSettingsRes && unitEconomicsSettingsRes.ok
				? await unitEconomicsSettingsRes.json()
				: null;
		const leadershipKpis =
			leadershipKpisRes && leadershipKpisRes.ok ? await leadershipKpisRes.json() : null;
		const dailyCosts = dailyCostsRes && dailyCostsRes.ok ? await dailyCostsRes.json() : null;
		const approvalsQueue =
			approvalsQueueRes && approvalsQueueRes.ok ? await approvalsQueueRes.json() : null;
		...
		return {
			...
			endDate,
			provider,
			warning: warningMessage.length > 0 ? warningMessage : undefined
		};
	} catch (err) {
