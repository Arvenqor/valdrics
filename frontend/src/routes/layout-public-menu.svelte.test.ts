...
describe('public layout mobile menu', () => {
	...
	async function renderAuthenticatedPublicLayout() {
	...
	it('keeps public routes in the public shell even for authenticated users', async () => {
		const { container } = await renderAuthenticatedPublicLayout();

		expect(container.querySelector('.public-site-shell')).toBeTruthy();
		expect(screen.queryByRole('button', { name: /open command palette/i })).toBeNull();
