...
describe('EnforcementSettingsCard', () => {
	...
	it('loads and saves policy, budget, and credit endpoints', async () => {
		...
		expect(screen.getByText('Blocking rails')).toBeTruthy();
		expect(screen.getByText('Approval scopes')).toBeTruthy();
		expect(screen.getByText('1/2')).toBeTruthy();
		expect(
			screen.getByText(
				'Hard rails block failing requests, soft rails route them through review, and shadow rails record evidence without changing the approval decision.'
			)
		).toBeTruthy();
		expect(screen.getByText('Kubernetes admission gate')).toBeTruthy();
		...
		expect(
			getMock.mock.calls.some(([url]) => {
				...
				return path === endpoint('/policies') || path.startsWith(`${endpoint('/policies')}/`);
			})
		).toBe(false);
		...
		expect(screen.getByText('Budget Allocations')).toBeTruthy();
		expect(screen.getByText('Temporary Credits')).toBeTruthy();
