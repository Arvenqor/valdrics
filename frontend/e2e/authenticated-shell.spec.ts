...
test.describe('Authenticated shell', () => {
	...
	test('marks the active dashboard nav item and keeps persona extras collapsed by default', async ({
		...
		const topbar = page.locator('.authenticated-shell__topbar');
		await expect(sidebar).toBeVisible();
		await expect(topbar).toContainText(/valdrics workspace/i);
		await expect(topbar).toContainText(fixtureTierPattern());
