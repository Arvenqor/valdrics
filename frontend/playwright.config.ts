...
function readFrontendEnvValue(name: string): string {
	try {
...
const defaultE2EEnv = {
	...
	PLAYWRIGHT_E2E_DATABASE_URL: 'sqlite+aiosqlite:////tmp/valdrics-frontend-playwright.sqlite3',
...
const publicSupabaseUrl =
	process.env.PUBLIC_SUPABASE_URL || readFrontendEnvValue('PUBLIC_SUPABASE_URL');
...
if (!isPublicOnly) {
	webServer.push({
		command: `cd .. && ${backendEnv} uv run python3 scripts/run_frontend_playwright_backend.py --host 127.0.0.1 --port 8000`,
		port: 8000,
		reuseExistingServer: false
	});
}
