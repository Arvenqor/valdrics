...
The default compose profile is intended for offline local development with
PostgreSQL and no Redis dependency. An isolated Redis overlay can be enabled
explicitly for local cache/coordinator drills. This file must never be treated
...
def _render_env(template_lines: list[str], overrides: dict[str, str]) -> str:
    ...
    output_lines: list[str] = [
        ...
        "# Default compose path is cacheless. Use `make docker-up-redis` only when you need the isolated Redis drill overlay.",
