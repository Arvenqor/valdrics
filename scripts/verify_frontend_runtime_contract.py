#!/usr/bin/env python3
"""Verify the frontend container runtime contract used by managed deployments."""
...
def _required_paths(root: Path) -> tuple[Path, ...]:
    ...
    build_root = frontend_root / ".svelte-kit" / "output"
    return (
        root / "Dockerfile.frontend",
        frontend_root / "server.node.mjs",
...
def _text_contract_errors(root: Path) -> list[str]:
    errors: list[str] = []
    dockerfile_path = root / "Dockerfile.frontend"
    server_path = root / "frontend" / "server.node.mjs"
    ...
    if "COPY --from=builder /app/.svelte-kit/output ./build" not in dockerfile_text:
        errors.append(
            "Dockerfile.frontend must copy .svelte-kit/output into ./build for the runtime image."
        )
    if 'CMD ["node", "server.node.mjs"]' not in dockerfile_text:
        errors.append("Dockerfile.frontend must start the frontend with node server.node.mjs.")
    if "await app.init({ env: process.env });" not in server_text:
        errors.append("frontend/server.node.mjs must initialize the SvelteKit server with env.")
    if "import { Server } from './build/server/index.js';" not in server_text:
        errors.append("frontend/server.node.mjs must import the built server entrypoint.")
    if "import { manifest } from './build/server/manifest.js';" not in server_text:
        errors.append("frontend/server.node.mjs must import the built server manifest.")
    return errors
...
def _frontend_runtime_lock_path(root: Path) -> Path:
    return root / ".runtime" / "locks" / "frontend-runtime-contract.lock"


@contextmanager
def _frontend_runtime_lock(
    ...
    lock_path = _frontend_runtime_lock_path(root)
        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise TimeoutError(
                        "frontend runtime contract lock could not be acquired within "
...
@contextmanager
def _prepared_frontend_runtime_root(root: Path):
    frontend_root = root / "frontend"
    output_root = frontend_root / ".svelte-kit" / "output"
    ...
    if build_root.exists() or build_root.is_symlink():
        backup_root = runtime_tmp_root / f"frontend-build-backup-{uuid.uuid4().hex}"
...
def smoke_frontend_runtime(
    root: Path,
    *,
    timeout_seconds: float = DEFAULT_TIMEOUT_SECONDS,
) -> str | None:
    with _prepared_frontend_runtime_root(root) as frontend_root:
        ...
        try:
                ...
                try:
                    with urlopen(f"{origin}/", timeout=2) as response:
                        ...
                        if response.status == 200 and "<html" in body.lower():
                            return None
                        last_error = (
                            f"unexpected frontend response status={response.status} "
            ...
            if not last_error:
                if process.poll() is not None:
                    last_error = (
                        f"frontend runtime exited early with code {process.returncode}"
                    )
                else:
                    last_error = "frontend runtime did not become ready before timeout"
...
def _format_smoke_failure(process: subprocess.Popen[str], message: str) -> str:
    ...
    if log_excerpt:
        return f"frontend runtime smoke failed: {message}. logs={log_excerpt!r}"
    return f"frontend runtime smoke failed: {message}"


def verify_frontend_runtime_contract(
    *,
    root: Path = DEFAULT_ROOT,
    build: bool = False,
    skip_smoke: bool = False,
    lock_timeout_seconds: float = DEFAULT_LOCK_TIMEOUT_SECONDS,
    build_runner: BuildRunner = build_frontend,
    ...
    try:
        with _frontend_runtime_lock(
            ...
            for path in _required_paths(repo_root):
                if not path.exists():
                    errors.append(
                        f"missing required frontend runtime artifact: {path.as_posix()}"
                    )
                elif not path.is_file():
                    errors.append(
                        f"frontend runtime artifact must be a file: {path.as_posix()}"
                    )
            ...
            runner = smoke_runner or (lambda root_path: smoke_frontend_runtime(root_path))
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Verify the frontend managed-runtime contract by checking the built output, "
    ...
    parser.add_argument(
        "--build",
        action="store_true",
        help="Run `pnpm --dir frontend run build` before verifying the runtime contract.",
    )
    parser.add_argument(
        "--skip-smoke",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = verify_frontend_runtime_contract(
        root=root,
        build=bool(args.build),
        skip_smoke=bool(args.skip_smoke),
    )
    if errors:
        print("[frontend-runtime] FAILED")
        for error in errors:
            print(f"- {error}")
        return 1
