...
| Canonical normalized ledger model | Partial | `PLAN.md`, `app/modules/reporting/domain/persistence.py` | The internal model and persistence shape support normalized cost data, but this is not yet proven as a mature single ledger across every technology category described in the feedback. |
...
| --- | --- | --- | --- |
| Unified technology spend ledger | Partial | `docs/compliance/focus_export.md`, `PLAN.md`, `app/models/saas_connection.py`, `app/models/license_connection.py`, `app/models/platform_connection.py`, `app/models/hybrid_connection.py`, `app/models/llm.py` | Valdrics already spans cloud-plus connectors and LLM telemetry, but it is not yet evidenced as a fully mature single ledger across all spend domains. |
