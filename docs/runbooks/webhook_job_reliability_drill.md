...
This runbook defines the release-critical reliability drill for the Paystack
webhook ingestion path and the downstream `background_jobs` processor on the
unified platform.
...
3. Force a managed processing failure while jobs are `RUNNING` and verify stale-job recovery requeues or dead-letters abandoned work.
...
6. Verify managed dispatch failure is visible through Cloud Logging and the
   fail-closed internal scheduler dispatch contract.
...
2. Scenario-specific pytest targets covering duplicate redelivery, stale recovery, dead-letter routing, retention cleanup, and the managed scheduler dispatch contract.

## Staging Drill

Use staging for the full operational rehearsal after the deterministic local drill passes.

1. Run the staging release workflow and confirm the reusable deploy job applies the managed migration env successfully:
...
4. Simulate a Cloud Tasks or Cloud Run execution interruption while jobs are
   `RUNNING`, then verify the repository-managed worker loop requeues or
   dead-letters them.
...
Confirm these Cloud Monitoring alert policies exist before production promotion:
...
- `SchedulerDispatchFailClosedDetected`
...
2. The staging release workflow migration step completes successfully.
...
5. Cloud Logging, Cloud Monitoring alerts, and dashboard panels show the
   expected managed-runtime signals during the drill window.
...
including the reusable deploy workflow migration step from `.runtime/production.migrate.env`,
